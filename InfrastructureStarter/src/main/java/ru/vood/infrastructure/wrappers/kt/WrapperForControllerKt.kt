...
interface WrapperForController : BeginnerOfChainFunctionInterface {
    ...
    fun <R> validError(errorMessage: ErrorMessage) = WrappedObject<R>(CommonStatus.ERROR, null, errorMessage, null)

    fun <R> getError(e: Exception) = WrappedObject<R>(CommonStatus.ERROR, null, ErrorMessage(listOf<String>(e.message!!)), null)

    fun <R> getOk(objectFromService: WrapperForService.WrappedObject<R>) =
            WrappedObject<R>(CommonStatus.OK,
                    RestContext(Date(),
